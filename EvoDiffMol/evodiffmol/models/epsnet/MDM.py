...
class MDMFullDP(nn.Module):
    ...
    def net(self, atom_type, pos, bond_index, bond_type, batch, time_step,
            edge_index=None, edge_type=None, edge_length=None, context=None, vae_noise=None, update_mask=None):
        """
        Args:
    ...
    def forward(self, batch, context=None, return_unreduced_loss=False, return_unreduced_edge_loss=False,
                extend_order=True, extend_radius=True, is_sidechain=None, update_mask=None):
        ...
        """
        Independently
        - Perterb pos (respecting scaffold mask)
        """
        pos_noise = torch.zeros(size=pos.size(), device=pos.device)
        pos_noise.normal_()
        
        if update_mask is not None:
            # Only add noise to non-scaffold atoms (update_mask == 1)
            noise_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            pos_perturbed = pos + center_pos(pos_noise, batch) * (1.0 - a_pos).sqrt() / a_pos.sqrt() * noise_mask
        else:
        ...
        """
        Perterb atom (respecting scaffold mask)
        """
        ...
        if update_mask is not None:
            # Only add noise to non-scaffold atom types (update_mask == 1)
            noise_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            atom_perturbed = a_pos.sqrt() * atom_type + (1.0 - a_pos).sqrt() * atom_noise * noise_mask
        else:
        ...
        # Apply scaffold mask to position loss - only compute loss for non-scaffold atoms
        if update_mask is not None:
            pos_loss_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            loss_global = loss_global * pos_loss_mask
        ...
        # Apply scaffold mask to local position loss
        if update_mask is not None:
            pos_loss_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            loss_local = loss_local * pos_loss_mask

        loss_local = torch.sum(loss_local, dim=-1, keepdim=True)

        loss_node_global = (node_score_global - atom_noise) ** 2
        # Apply scaffold mask to atom type loss - only compute loss for non-scaffold atoms
        if update_mask is not None:
            atom_loss_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            loss_node_global = loss_node_global * atom_loss_mask
        loss_node_global = torch.sum(loss_node_global, dim=-1, keepdim=True)

        loss_node_local = (node_score_local - atom_noise) ** 2
        # Apply scaffold mask to local atom type loss
        if update_mask is not None:
            atom_loss_mask = update_mask.unsqueeze(-1)  # Shape: (N, 1)
            loss_node_local = loss_node_local * atom_loss_mask
        loss_node_local = torch.sum(loss_node_local, dim=-1, keepdim=True)
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, context,
                                 ...
                                 w_global_node=1, w_local_pos=1, w_local_node=1, update_mask=None,
                                 start_timestep=None, **kwargs):
        ...
        with torch.no_grad():
            # Store initial atom types and positions for fixed structure masking
            atom_type_init = atom_type.clone() if update_mask is not None else None
            pos_init_original = pos_init.clone() if update_mask is not None else None  # Store before centering
            
            if start_timestep is not None:
                seq = range(self.num_timesteps - start_timestep, self.num_timesteps)
            else:
                seq = range(self.num_timesteps - n_steps, self.num_timesteps)
            seq_next = [-1] + list(seq[:-1])
            
            # Only center positions if no fixed structure, otherwise centering destroys fixed scaffold positioning
            pos = center_pos(pos_init, batch)  # Normal centering for standard generation
            # if update_mask is None:
            #     pos = center_pos(pos_init, batch)  # Normal centering for standard generation
            # else:
            #     pos = pos_init.clone()  # Dont center for the whole molecule

                # # The following may be useful
                # # Center only the movable atoms while keeping fixed atoms in place
                # movable_mask = (update_mask == 1)  # Movable atoms have mask = 1
                # if movable_mask.any():
                #     # Create a sub-batch for movable atoms to use center_pos function
                #     movable_indices = movable_mask.nonzero(as_tuple=True)[0]
                #     movable_pos = pos[movable_indices]
                #     movable_batch = batch[movable_indices]  # Get batch assignments for movable atoms
                    
                #     # Use center_pos function to properly center movable atoms
                #     centered_movable_pos = center_pos(movable_pos, movable_batch)
                #     pos[movable_indices] = centered_movable_pos
            
            # VAE noise
            vae_noise = torch.zeros_like(atom_type).uniform_(-1, +1)

            # Create progress bar that shows reverse timestep
            pbar = tqdm(zip(reversed(seq), reversed(seq_next)), total=len(seq), desc='reverse timestep')
            for i, j in pbar:
                # Update progress bar to show current reverse timestep
                pbar.set_postfix({'timestep': f'{i}/{self.num_timesteps-1}'})
                ...
                # Apply update mask to keep fixed atoms unchanged (critical for fixed structures)
                if update_mask is not None:
                    # For fixed atoms (update_mask == 0): keep original positions and atom types
                    # For updateable atoms (update_mask == 1): use updated positions and atom types
                    
                    # STRONG ENFORCEMENT: Hard reset fixed atom positions to their exact original values
                    # This overrides any drift that might have occurred during the network computation
                    fixed_indices = (update_mask == 0).nonzero(as_tuple=True)[0]
                    if len(fixed_indices) > 0:
                        pos_next[fixed_indices] = pos_init_original[fixed_indices]
                    
                    # Also hard reset fixed atom types
                    atom_next = atom_type_init * (1 - update_mask.unsqueeze(-1)) + atom_next * update_mask.unsqueeze(-1)
                ...
                # # Only center positions if no fixed structure, otherwise centering destroys fixed positions
                # if update_mask is None:
                #     pos = center_pos(pos, batch)
