...
class PolyNation:
    ...
    def __init__(self, name: str, land: PolyLand, 
                 ...
                 diversity_similarity_threshold: float = 0.5,
        ...
        if diversity_similarity_threshold < 0:
            logging.info('diversity similarity threshold was {}, switched to 0'.format(
                         diversity_similarity_threshold))
            diversity_similarity_threshold = 0.0
        if diversity_similarity_threshold > 1:
            logging.info('diversity similarity threshold was {}, switched to 1'.format(
                         diversity_similarity_threshold))
            diversity_similarity_threshold = 1.0
        self.diversity_similarity_threshold = float(diversity_similarity_threshold)
        self.land.add(self)
        self.random_seed = random_seed
        # set random seed
        if self.random_seed != 0:
                ...
                for index, row in df.iterrows():
                    ...
                    for index2, row2 in df.iterrows():
                        if index != index2:
                            ...
                            if score == -1:
                                continue
                            if score < self.diversity_similarity_threshold: 
    ...
    def __mutate(self, chromosome_ids):
        ...
        """
        # EGD diffusion-based mutation (when available on the land).
        egd_mutator = getattr(self.land, 'egd_mutator', None)
        if egd_mutator is not None:
            egd_prob = getattr(self.land, 'egd_mutation_prob', 0.3)
            if self.rng.random() < egd_prob:
                result = self.__egd_mutate(chromosome_ids, egd_mutator)
                if result is not None:
                    return result
        # Original random mutation (fallback)
    ...
    def __egd_mutate(self, chromosome_ids, egd_mutator):
        """EGD diffusion-based mutation for one position in *chromosome_ids*.

        Picks a random position, retrieves its fragment SMILES from the
        planet's chromosome registry, calls the EGD mutator, registers
        any new fragments back into the registry with fresh IDs, and
        replaces the mutated position.

        Returns a modified *chromosome_ids* list, or None on any failure
        ...
        try:
            ...
            pos = int(self.rng.integers(0, len(chromosome_ids)))
            frag_id = chromosome_ids[pos]

            # Retrieve the fragment SMILES from the planet's registry.
            frag_smiles = self.land.planet.chromosomes.get(frag_id)
            if frag_smiles is None:
                return None

            # Run EGD mutation.
            new_fragments = egd_mutator.mutate_fragment(frag_smiles)
            if not new_fragments:
                return None

            # Register new fragments in the planet's chromosome dict.
            new_ids = []
            for smi in new_fragments:
                new_id = self.land.planet.uid()
                self.land.planet.chromosomes[new_id] = smi
                new_ids.append(new_id)

            # Replace the mutated position with the first new fragment.
            chromosome_ids[pos] = new_ids[0]
            return chromosome_ids
        except Exception:
            return None
