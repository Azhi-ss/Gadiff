    ...
    def __init__(self, args, dictionary):
        ...
        if os.path.isfile(local_vocab_path) and os.path.isfile(local_merges_path):
            self.tokenizer = PolymerSmilesTokenizer(
                ...
                merges_file=local_merges_path,
            )
        else:
            # Fallback to strictly local cache of roberta-base without network access
            self.tokenizer = PolymerSmilesTokenizer.from_pretrained(
                "roberta-base", max_len=411, local_files_only=True
            )        
        # Build a local RobertaConfig to avoid remote lookups
